...
def process_combat_segment(active_segment: dict, segment_def: dict, character: dict) -> tuple:
    ...
    for round_num in range(max_rounds):
        ...
        char_off_result = resolve_opposed_check_with_xp(
            character_id, char_off_rating, opp_def_rating, char_off_skill, char_off_attr, xp_accumulator
        )
        ...
        opp_off_result = resolve_opposed_check_with_xp(
            character_id, char_def_rating, opp_off_rating, char_def_skill, char_def_attr, xp_accumulator
        )
        ...
        if (
            ...
            logger.info(
                f"Opponent defeated in round {round_num + 1} (outcome: {outcome}, character wounds: {player_wound_count})"
            )
            return outcome, {
                "Rounds": round_num + 1,
                "PlayerWounds": player_wounds,
