...
**Why Currency Doesn't Display:**
- Flutter has complete currency display implementation
- Backend sends `Resources: {}` (empty dict)
- Empty dict means `isNotEmpty` is false → section hidden
- **Backend never populates Resources field**
...
- Store screen (when api_store_* endpoints exist)
