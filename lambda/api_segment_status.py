...
def build_base_status_response(active_segment: dict, now: float) -> tuple:
    """Compute segment timing and build the base status response.

    Returns ``(response, processing_status)``. Duration falls back to 60 seconds
    when neither stored times nor a duration field yield a positive value.
    """
    ...
    # Client uses ProcessingStatus to know when the backend is done and the local
    # timer (EndTime/TimeRemaining) to know when to display results.
    if processing_status != "processed":
        next_poll_delay = min(RETRY_POLL_DELAY, time_remaining)  # Cap at time remaining
        poll_after = from_unix(int(now + next_poll_delay))
    else:
...
def enrich_processed_result(response: dict, active_segment: dict, story_id, segment_id, segment_type: str) -> dict:
    ...
    try:
        ...
        # Story is complete if there is no next segment
        ...
        logger.warning(f"Failed to get narrative data for segment {segment_id}: {err.__class__.__name__}: {err}")
        # Continue without narrative - not critical

    return segment_def


def enrich_segment_metadata(response: dict, segment_def, story_id, segment_id) -> dict:
    """Fill SegmentTitle / SegmentActivity from the segment definition when missing.

    Returns the (possibly fetched) segment definition for reuse.
    """
...
def enrich_decision_data(response: dict, active_segment: dict, segment_def, story_id, segment_id) -> dict:
    """Add decision text and client-safe options for a decision segment.
    ...
    if segment_def:
        ...
        response["DecisionOptions"] = filter_decision_options(segment_def.get("DecisionOptions", {}))
    elif active_segment.get("DecisionOptions"):
        # Fallback to active segment data (filter same fields)
        response["DecisionOptions"] = filter_decision_options(active_segment.get("DecisionOptions", {}))
    return segment_def


def enrich_story_info(response: dict, story_id) -> None:
    """Add story title/description/type to the response. Non-fatal on failure."""
    try:
...
def get_segment_status_business_logic(character_id: str, player_id: str) -> dict:
    ...
    try:
        ...
        if "No active story found" in str(err):
            ...
            raise NotFoundError("No active segment found") from err
        raise err

    response, processing_status = build_base_status_response(active_segment, time.time())

    # Include result data if the segment is fully processed (client controls display)
    ...
    segment_def = enrich_segment_metadata(response, segment_def, story_id, segment_id)

    if segment_type == "decision":
        enrich_decision_data(response, active_segment, segment_def, story_id, segment_id)

    if story_id:
        enrich_story_info(response, story_id)

    logger.info(
        f"Segment status for character={character_id}, segment={response.get('ActiveSegmentID')}: "
        f"Status={response.get('Status')}, ProcessingStatus={response.get('ProcessingStatus')}, "
        f"TimeRemaining={response.get('TimeRemaining')}s"
    )
