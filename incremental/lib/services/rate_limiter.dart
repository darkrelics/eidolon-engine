...
class RateLimiter {
  ...
  Future<T> executeHumanDriven<T>(
    ...
    if (lastCall != null) {
      ...
      if (remaining > Duration.zero) {
        debugPrint('RateLimiter: Action $key rate limited, ${remaining.inSeconds}s remaining');
  ...
  Future<T> executeAutomated<T>(
    ...
    if (lastCall != null) {
      ...
      if (remaining > Duration.zero) {
        debugPrint('RateLimiter: Automated action $key delayed by ${remaining.inSeconds}s');
  ...
  void clearLimit(String key) {
    ...
    _pendingTimers.remove(key);
    _pendingCompleters[key]?.completeError(
      RateLimitException('Cleared', remaining: Duration.zero),
    );
    _pendingCompleters.remove(key);
  }
