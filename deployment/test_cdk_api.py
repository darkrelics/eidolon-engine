...
def test_cdk_api():
    ...
    try:
        # Initialize CDK API
        cdk_api = CDKApiIntegration(cdk_dir=str(Path(__file__).parent / "cdk"), region="us-east-1")
        ...
        result = cdk_api.synth(context={"deploy_mud": "true", "deploy_incremental": "false"})
        if result["success"]:
        ...
        print("\nTesting CDK diff...")
        diff_result = cdk_api.diff(context={"deploy_mud": "true", "deploy_incremental": "false"})
