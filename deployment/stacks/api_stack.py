...
class ApiStack(Stack):
    ...
    def _add_api_endpoints(self, api: apigateway.RestApi, authorizer: apigateway.CognitoUserPoolsAuthorizer) -> None:
        ...
        # Item endpoints
        item_resource = api.root.add_resource("item")

        if "api-item-brief" in self.lambda_arns:
            brief_resource = item_resource.add_resource("brief")
            self._add_lambda_integration(brief_resource, "GET", "api-item-brief", authorizer)

        if "api-item-prototype" in self.lambda_arns:
        ...
        if "api-item-split" in self.lambda_arns:
            split_resource = item_resource.add_resource("split")
            self._add_lambda_integration(split_resource, "POST", "api-item-split", authorizer)
