...
def grant_cognito_permission(lambda_function_name, user_pool_id):
    """Grant Cognito permission to invoke the Lambda function."""
    lambda_client = boto3.client('lambda', region_name='us-east-1')
    
    # Construct the source ARN for Cognito
    account_id = boto3.client('sts').get_caller_identity()['Account']
    source_arn = f"arn:aws:cognito-idp:us-east-1:{account_id}:userpool/{user_pool_id}"
...
def update_user_pool_triggers(user_pool_id, lambda_function_name):
...
def verify_configuration(user_pool_id, lambda_function_name):
